...
export default class VatomView {
  ...
  load() {
    ...
    const du = st.properties.display_url;
    ...
    if (FaceClass === undefined && du.indexOf('http') !== -1) {
      FaceClass = ImageFace;
    } else if (FaceClass === undefined && du.indexOf('http') === -1) {
    ...
    const rFace = new FaceClass(this, this.vatomObj, st);
    ...
    Promise.resolve(() => null).then(() => rFace.onLoad()).then(() => {
  ...
  static registerFace(faceClass) {
    registeredFace[faceClass.url] = faceClass;
  }
}
